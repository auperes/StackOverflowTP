...
<html ondragstart='event.preventDefault();'>
	...
	<body>
		<ul class="gn-menu-main">
			...
			<li><a href="${createLink(uri: '/question/index')}">${message(code: 'menu.questions')}</a></li>
			<li><a href="${createLink(uri: '/answer/index')}">${message(code: 'menu.answers')}</a></li>

			<g:if test="${session.user}">
				<li class="last-child"><a href="${createLink(uri: '/user/logout')}" id="deconnexion">${message(code: 'menu.logout')}</a></li>
				<li class="last-child"><a href="${createLink(uri: '/user/show')}/${session.user.id}">${message(code: 'menu.profile')}</a></li>
			</g:if>
			<g:else>
